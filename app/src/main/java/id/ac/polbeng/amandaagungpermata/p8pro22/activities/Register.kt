    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSubmit.setOnClickListener {
            ...
            if(!password.equals(konfirmasiPassword)){
                etKonfirmasiPassword.error = "Password dan konfirmasi password tidak sama!.requestFocus()"
                        return@setOnClickListener
            }
            ...
            requestCall.enqueue(object : Callback<DefaultResponse>{
                override fun onFailure(call: Call<DefaultResponse>, t: Throwable) {
                    Toast.makeText(this@Register, "Error terjadi ketika sedang mendaftarkan user: " + t.toString(), Toast.LENGTH_LONG).show()
                }
                ...
                override fun onResponse(
                    ...
                    if(!response.body()?.error!!) {
                        ...
                        defaultResponse.let {
                            Toast.makeText(this@Register, defaultResponse.message, Toast.LENGTH_LONG).show()
                    val intent = Intent(applicationContext, Login::class.java)
                            intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
                            startActivity(intent)
                        }
                    }else{
                        Toast.makeText(this@Register, "Gagal mendaftarkan user: " + response.body()?.message, Toast.LENGTH_LONG).show()
                    }
                }
            })
        }
